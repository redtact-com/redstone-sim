...
// ── ライブ観測 (#366) ────────────────────────────────────────────
// 1 回の呼び出しで **いまの region をまるごと** shared/live.json へ書く。
// キャプチャ側の差分機構 (__on_tick) は `/tick step N` をまとめて撃つ前提だが、
// ライブは 1 tick ずつ人が触るので、素の全走査で足りる
// (差分はホスト側が前回との比較で作る)。
fx_live_save() -> (
  write_file('live', 'shared_json', {'blocks' -> _scan_region()});
  'ok'
);
