...
`captures/elev-call1.def.json` / `elev-call-high.def.json` が**動かし方の見本**。

- 入力は**各階の呼びボタン** (`1,{59,64,69,74,87,99,105,112,120,138},8` の
  `oak_button[face=wall,facing=north]`)。`players[0]` をその階のかご内
  (`1.5, 階-2, 7.5`) に湧かせておく
- 「動いた」と言える観測点は **その階の鉄のドア** (`4,階-2,15` / `4,階-1,15`)。
  tick 14 で開き tick 19 で閉じる。副次的に銅の電球が点く
- **ピストンやかごは動かない**。ソウルサンドの上昇流で人が昇る機械なので、
  泡柱 560 本は常に上向き (`drag=false`) のまま
- 塀が縦一列 (53〜186 マス) まとめて `up` を反転するのはバニラの挙動 (#244)。
  ドアの接続が外れた拍子に下まで連鎖する
- #239 で「制御盤から先へ伝わらない」と記録されていたのは、**狙点がセル中心で
  壁付けのボタンを押せていなかった**ため (#378 で解決)
