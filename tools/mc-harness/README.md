...
`captures/elev-floor3.def.json` が**動かし方の見本**。`elev-call1` / `elev-call-high`
はボタンだけ押す短い版。

- **呼び出しではなく階の指定**。各階に書見台があり (`1,{57,62,67,72,85,97,103,110,118,136},9`)、
  **その `Page` が行き先の階** (0 始まり / 全 15 ページ)。地上の制御盤にも 1 台ある (`5,2,8`)
- 手順は **書見台のページを変える → その階のボタンを押す** の 2 手。
  ボタンは `1,{59,64,...},8` の `oak_button[face=wall,facing=north]`
- **同じ階を指定すると何も動かない**。かごが 1F (y=56〜60) に居るとき 1F を選んでも
  ドアが開閉するだけで終わる (これを「制御盤から先へ伝わらない」と誤診していた)
- 「動いた」と言える観測点は **かご (`honey_block`) の y**。
  1 階分 = 5 ブロックで、**1 ブロックあたり 10 tick**。1F → 3F は tick 66 で動き出して
  tick 156 に y=66〜70 で止まる (10 ブロック / 90 tick)
- その階の鉄のドア (`4,階-2,15` / `4,階-1,15`) が開閉するのは**到着の合図**。
  ボタンを押してから 12〜14 tick で開き、その 5 tick 後に閉じる
- 泡柱 4 本 (`x=3,z=1` / `x=5,z=1` / `x=5,z=14` / `x=5,z=15`・各 140 段) はソウルサンド
  (`y=5`) が源。**移動中に一度 water へ崩れて戻る** (tick 54 で崩れ tick 143 で復活)
- `players[0]` はかご内 (`1.5, 階-2, 7.5`) に湧かせる。**壁付けのボタンはそこから押せる** (#378)
