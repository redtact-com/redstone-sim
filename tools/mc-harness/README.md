...
- **壁付けのボタン/レバーは fake player では押せない** (#376 で再確認)
  - 症状: `use once` は通るのに `powered` が `false` のまま。ブロック中心を狙っても、
    外側の面から狙い直しても変わらない
  - エレベーターの呼びボタン (`1,59,8` の `oak_button[face=wall]`) がこれで、
    **機械が動くかどうかをハーネスからは確かめられない**。人が入って押すしかない
    (「クライアントで入る」の節)
