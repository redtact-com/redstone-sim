...
| 分類 | ブロック |
|---|---|
| 入力 | レバー / ボタン（石・木）/ 感圧板（石・木・重量式 軽・重）/ ターゲット |
| 伝送 | ワイヤー / トーチ（床・壁）/ リピーター（遅延 1–4）/ コンパレーター（比較・差引）|
| 出力・機構 | ランプ / 音符ブロック / ピストン（通常・粘着）/ オブザーバー / レッドストーンブロック |
| ブロック | 固体ブロック / スライム / ハチミツ |
| レール | レール / パワードレール / アクティベーターレール / ディテクターレール |
| 物流 | コンテナ / ホッパー / ドロッパー |

**非対応**（[スコープ方針](docs/research/13_scope-policy.md)）: トロッコ・モブ・アイテムエンティティ等の
エンティティ全般、ブロックの支持要件（浮いたままになる）、音の再生。
レールは敷けますがトロッコは走りません（レッドストーン素子としての側面のみ実装）。
...
> **要判断**: このリソースパックは**再配布の許諾を確認できていません**。
> 作者から許可を得るか、同梱をやめる (フォールバックだけにする) か、自前アートへ
> 置き換えるかを決める必要があります。詳細は [docs/ip-policy.md](docs/ip-policy.md) §2。
