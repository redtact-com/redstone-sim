...
| 項目 | 内容 |
|---|---|
| URL | https://carpet.tis.world/docs/loggers / https://github.com/TISUnion/Carpet-TIS-Addition / https://modrinth.com/mod/carpet-tis-addition |
| カバー範囲 | tick 内の block update (detect/emit/schedule)・tile tick (schedule/execute)・block event (schedule/execute)・block state change/replace・piston push-structure 計算を**発生順に 1 イベント 1 行**で出力。イベントクラス実体 = `microtiming/events/` 配下の DetectBlockUpdateEvent / EmitBlockUpdateEvent (+RedstoneDust 版) / ScheduleBlockUpdateEvent / ScheduleTileTickEvent / ExecuteTileTickEvent / ScheduleBlockEventEvent / ExecuteBlockEventEvent / BlockStateChangeEvent / BlockReplaceEvent / PistonComputePushStructureEvent [確定: ソース直読 https://github.com/TISUnion/Carpet-TIS-Addition/tree/master/src/main/java/carpettisaddition/logging/loggers/microtiming/events] |
| 由来 | 拡張作者 Fallen_Breath (TISUnion) の Mixin 実装、公式 docs |
| 信頼度 | 高。**1.21.1 実機ロード + `/carpet microTiming` 系ルールの存在を実測確認済み** [確定 2026-07-02 実機: fabric-carpet 1.4.147+v240613 + carpet-tis-addition 1.81.0-mc1.21.1]。出力の実「行」は本調査では未採取 (headless 制約、下記) |
| ライセンス | **LGPL-3.0-only** [確定: Modrinth API `license` フィールド https://api.modrinth.com/v2/project/carpet-tis-addition + GitHub バッジ]。外部 mod として実行・観測は自由。派生 mixin を配布する場合は LGPL 条件 (ソース開示・同一ライセンス継承) に従う。**観測された更新順という「事実」の fixture 化は著作権対象外で自由** |
| 対応バージョン | **1.21.1 対応 [確定: Modrinth API]**。release 版 v1.74.3〜v1.81.0 が全て game_versions=[1.21,1.21.1]/loader=fabric/type=release。1.14.4〜26.2-snapshot まで広くカバー。server_side=optional (サーバ単体で動作) |
| 使い方 | P3。**update「順序」の ground truth 取得の本命** (carpet 本体・SubTick では取れない neighbor update 順が取れる)。**ただし出力は subscribed player の chat 限定で rcon-only の mc-harness では取得不可** — `/log` を console 実行すると "players only"、`/log microTiming <name>` は "No player specified"、fake player を spawn しても購読不可、lever トグルの block update も server log に一切出ない [確定 2026-07-02 実機]。自動化には (a) グラフィカルクライアント手動観察、または (b) `MicroTimingLoggerManager` にフックする補助 mixin で JSON 出力、のいずれかが必要。詳細評価は **06_tool-eval.md** |
...
## 17. Redstone Multimeter (RSMM, Space Walker / SpaceWalkerRS)

| 項目 | 内容 |
|---|---|
| URL | https://github.com/SpaceWalkerRS/redstone-multimeter-fabric / https://modrinth.com/mod/redstone-multimeter |
| カバー範囲 | クライアントのホットキーでブロックに「meter」を設置し、各 tick の powered / activated / moved (piston) / ticked などの状態遷移を**画面上の HUD タイムライン**として可視化 (緑=powered / 赤=unpowered / 横線=移動)。meter 単位で追跡イベント種別・色・位置を設定でき、**同 tick 内の発火順 (subtick 順) と tick phase 情報も表示**する。NarcolepticFrog 版のマルチプレイ移植 |
| 由来 | Space Walker のソース + 公式 wiki (一次資料) |
| 信頼度 | 高。バージョン・ライセンスは Modrinth/GitHub で確認 [確定]。1.21.1 で「機械 diff に載せられる text/JSON エクスポートが無い」点は文書化されていないための推定 [要検証: 確認範囲でエクスポート機能は未文書。純 HUD 出力] |
| ライセンス | **MIT** [確定: GitHub] → コード・文書の参照/流用可 (帰属表示) |
| 対応バージョン | MC 1.17〜1.21.4 (最新 v1.17.x)。**1.21.1 対応** [確定: Modrinth version list、`[1.21]` ビルド mc1.21-1.16.0 が 1.21〜1.21.1 を、v1.17 系が 1.17〜1.21.4 をカバー]。Fabric / Quilt / Ornithe。**client+server 両方に導入必須**でグラフィカルクライアントが要る |
| 使い方 | P3 レイヤ C の**対抗馬**。更新順は見えるが出力が視覚 HUD のみ・block update type (NC/PP/CU) や tile tick priority のラベルが無く、機械 diff への組み込みは microTiming より難しい。ライセンス面 (MIT) は microTiming (LGPL) より緩い。詳細評価は **06_tool-eval.md** |

## 18. バニラ GameTest フレームワーク (/test, net.minecraft.gametest.Main)

| 項目 | 内容 |
|---|---|
| URL | https://minecraft.wiki/w/GameTest / https://www.minecraft.net/en-us/article/minecraft-snapshot-25w03a / https://minecraft.wiki/w/Java_Edition_1.21.5 |
| カバー範囲 | 1.21.5 (25w03a) で刷新された**バニラ標準**の E2E テスト機構。`/test run|runmultiple|runclosest|locate` コマンド + `test_instance` データパックレジストリ + ヘッドレスエントリ `net.minecraft.gametest.Main` (server.jar; `java -DbundlerMainClass="net.minecraft.gametest.Main"` で全テスト実行→exit、**mod 不要**)。テストは structure 配置 + 合否判定 |
| 由来 | Mojang 公式 (バニラ機能)。一次資料 |
| 信頼度 | 高 [確定: 公式チェンジログ + wiki]。ただし後述の「tick 系列は取れない」は設計上の帰結として [確定] |
| ライセンス | Minecraft EULA (公式機能。datapack 自作分は自身の著作物)。参照自由 |
| 対応バージョン | Java 1.21.5+ (26.x でも動作) |
| 使い方 | P3 補助 (合否ゲートのみ)。**mod なしバニラでは function test (assertBlockState/succeedWhen/runAtTickTime 等 GameTestHelper API) を登録できず** (`test_function` レジストリは Java=Mojang/mod からのみ) block-based test = structure + Test/Log/Fail/Accept ブロックを redstone で駆動する pass/fail 判定に限られる [確定: wiki]。**tick 単位の状態系列 (レイヤ A) も更新順 (レイヤ C) も取得不可**、取れるのは「特定時点で期待状態か」の合否のみ。→ 26.x の ground truth 系列生成器にはならず、scarpet(#6) の代替不可。詳細評価は **06_tool-eval.md** |

## 19. Mojira (bugs.mojang.com) — 公式バグトラッカー

| 項目 | 内容 |
|---|---|
| URL | 新形式 https://bugs.mojang.com/browse/MC/issues/MC-XXXXX (旧 https://bugs.mojang.com/browse/MC-XXXXX もリダイレクト有効)。fixture 候補バグ 7 件のカタログは 07_mojira-fixtures.md |
| カバー範囲 | 個別バグの表題・説明・Affected versions・Resolution (Fixed/Won't Fix/Awaiting Response 等)・Fix version・Labels (`experimental_redstone_fixed` 等)・重複リンク。redstone-sim では既知バグの fixture 候補 (再現/未修正の確認) と、experimental で挙動が変わる spec-drift の判定に使う |
| 由来 | Mojang 公式のバグトラッカー = 一次資料 (現象・影響版・修正状況の一次典拠)。ただしバグ本文はユーザ投稿を含み、根本原因の分析コメントは投稿者の推測を含む |
| 信頼度 | 表題・Affected versions・Resolution・Labels は一次資料で高 [確定]。**2024 年に JS レンダリング SPA へ移行済みで WebFetch は外枠しか取れない**ため、本調査は `https://r.jina.ai/<原票URL>` リーダで本文を取得。リーダは詳細サイドバー (一部の Affected versions/Labels) を取りこぼすことがあり、その分は検索スニペット補完 or [要検証]。旧 `bugs-legacy.mojang.com` は本調査時 ECONNREFUSED |
| ライセンス | 公式サイト利用規約。本文の転載は避け、事実 (現象・版・状況) の参照のみ |
| 使い方 | P1 (既知バグの現象・修正状況の一次典拠 → fixture 期待値) + P5 (`experimental_redstone_fixed` ラベル付きバグ = 将来 experimental 既定化で挙動が変わる spec-drift のウォッチ。例: MC-11193 locational)。07_mojira-fixtures.md に 7 件のカタログと skipUntil 想定を集約 |
