...
| G3 | 高 | **床置きトーチが水平給電しない** | facing=up トーチの横のワイヤー/ランプが power=0 (検証済) | トーチは取り付け面以外の全隣接に弱 15、直上のみ強充電 [確定: 02 §6 torch (I1)] | wire.ts:95-101, world.ts:597-606 | NOT ゲート横取り出しが全滅 |
...
| G5 | 高 | **ワイヤーが足元ブロックを弱充電しない** | 通電ワイヤー直下のランプ不点灯 (検証済) | dust は足元ブロック + connections 先を弱充電 [確定: 02 §5.4 (I1)] | world.ts:607-614 | 下方向配線が動かない |
...
| G7 | 中高 | **隣接更新の方向順が非準拠** | [north,south,east,west]+up/down 固定順、ワイヤーは独自 2 フェーズ BFS | NC: 西→東→下→上→北→南 DFS、素子別例外あり [確定: 02 §4.2 (I1)]。最終電力値は一致するはず [要検証: 未検証仮定] だが locational 回路が再現不能 | world.ts:372-452, types.ts:28-29 | 回路勢向け精度の分水嶺 |
...
| G12 | 低中 | **ボタン持続時間** | 石 5gt/木 10gt | 石 20gt/木 30gt [確定: 02 §6 (I1)] | world.ts:222-225 | 数値 1 行だが vanilla の 1/4 |
| G13 | 低中 | **レバー/ボタンの強充電が全隣接に効く** | facing 無視で全隣接 solid を強充電 → ブロック越しワイヤーが誤 15 | 強充電は取り付けブロックのみ [確定: 02 §6 lever/button (I1)] | world.ts:590-595 | 信号の「漏れ」方向非互換 |
...
1. **対象バージョンの確定 (I1 でまず決める)** — トレードオフ: (a) **1.20.1**: lntricate-SubTick が使える最終版 (レイヤ B 可) だが仕様が 3 世代古い。(a') **1.20.4**: MCHPRS の対応版だが **lntricate-SubTick は 1.20.1 までのため使えない** (レイヤ B 不可。批評#1 の矛盾を修正)。(b) **1.21.1**: 本調査のデコンパイル検証が最も厚い版。(c) **26.x**: 非難読化 jar で解析が最易だが実機検証 mod がほぼ皆無。推奨: **仕様典拠は 1.21.1 (+26.x jar を可読参照に併用)、ハーネスのレイヤ B のみ 1.20.1 で補助**とし、Orientation (WireOrientation) 差分は将来バージョンフラグで吸収 — I1 のデコンパイルで **1.21.2+ の刷新は experimental flag 付きで既定挙動は 26.2 現在も 1.21.1 と同一**と確定したため (02 §6 wire)、この方針のリスクは当初想定より小さい。1.20.1↔1.21.1 間の tile tick / block event 挙動差分は、レイヤ B 利用時に microTiming (レイヤ C) を 1.21.1 系でも並走させ、キュー消化順の同一性を fixture 単位で確認して吸収する。→ **決定済み (I1): 典拠 1.21.1 + 26.x 併読。CONTRIBUTING.md に明文化**。
