...
1. **rdsim の被埋め込み方針** — frame-ancestors 全開放のまま (推奨: 学習ツールとして
   個人ブログ等からの埋め込みも歓迎、13 §1 の価値 3 と整合) か、redtact 限定か
2. **Phase 2 (案 C) をやるか** — 埋め込みカードの共有性・redtact 外からの利用価値と、
   CORS/expiry のインフラ変更コストの天秤。Phase 1 完了後に判断でよい
3. **interact モードの範囲** — レバー/ボタンのトリガまで許可するか、view のみで始めるか
   (推奨: Phase 1 から interact を含める。手動トリガは 13 §2 の折衷モデルと同じ思想)
4. **embed の URL 形態** — ?embed=1 クエリ (App.tsx の分岐追加で最小) か /embed パス
   (ルーター導入) か。推奨はクエリ (現行の ?demo= と同型)
